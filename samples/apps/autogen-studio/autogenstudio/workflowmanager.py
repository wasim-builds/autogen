    ...
    def sanitize_agent(self, agent: Dict) -> Agent:
        ...
        if agent.config.llm_config is not False:
            ...
            for llm in agent.config.llm_config.config_list:
                ...
                if "api_key" not in llm and "OPENAI_API_KEY" not in os.environ and "AZURE_OPENAI_API_KEY" not in os.environ:
                    error_message = f"api_key is not present in llm_config or OPENAI_API_KEY/AZURE_OPENAI_API_KEY env variable for agent ** {agent.config.name}**. Update your workflow to provide an api_key to use the LLM."
